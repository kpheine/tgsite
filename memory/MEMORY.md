...
- Admin panel: configurable one-segment path from `ADMIN_PATH` (default `/painel-tg-2026`), password-protected via env-seeded admin user, SQLite sessions, case CRUD, drag-and-drop image upload with upload-time preview/order/removal/destaque selection, local video upload with admin preview, current video removal, individual image removal, and drag-and-drop existing image ordering. Case image cards use a shared Astro component for existing and pending uploads, with `Nova` / `Será removida` banners, button-based remove/restore, yellow bordered destaque toggle, and one shared ordering grid so newly uploaded images can be placed among existing images before saving. Public site is not connected to dashboard data yet.
