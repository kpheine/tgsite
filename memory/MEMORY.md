...
- Case uploads use UUID v4 filenames. Images and videos are hard-linked to cases for simplicity: deleting a case deletes its image/video DB rows and local files from `./uploads/`. Admin case upload endpoints parse multipart requests with `busboy` so large videos stream directly to disk instead of buffering the full file in memory.
